...
class KnowledgeGraphDataset(Dataset):
    ...
    def __init__(self, knowledge_list, lm_name="microsoft/deberta-v3-base"):
        ...
        print(
            "Gathered {} head-rel pairs and {} tails in total.".format(
                head_rel_cnt, tail_cnt
            )
        )
        self.inputs = self.tokenizer(
            self.input_strs, return_tensors="pt", padding=True, truncation=True
        )
...
def deberta_eval(deberta_model, kg_dataloader, kg_dataset, use_cuda=False):
    ...
    for batch in test_loop:
        ...
        with torch.no_grad():
            ...
            logit_tracker.extend(
                list(zip(head_rel_keys, tails, logits[:, 1].cpu().numpy()))
            )

    relevancy_score = float(relevancy_sum) / tot_seen

    for head_rel_key, tail, relevancy in logit_tracker:
        kg_dataset.head_rel_dict[head_rel_key]["tail_relevancy"].append(
            (relevancy, tail)
        )
...
def model_and_data_loading(args):
    ...
    kg_loader = DataLoader(kg_dataset, batch_size=args.eval_batch_size, shuffle=False)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=os.path.join("deberta_model_checkpoints", "checkpoint-454887"),
    ...
    relevancy_score, top1pick_list = deberta_eval(
        deberta_model, kg_loader, kg_dataset, use_cuda
    )
