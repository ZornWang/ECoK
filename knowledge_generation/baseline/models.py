...
class LSTMBinaryClassifier(nn.Module):
    ...
    def forward(self, input_ids, labels=None):
        h = torch.zeros((2 * self.lstm_layers, input_ids.size(0), self.hidden_dim)).to(
            self.device
        )
        c = torch.zeros((2 * self.lstm_layers, input_ids.size(0), self.hidden_dim)).to(
            self.device
        )
    ...
    def load_glove_embedding(self, args, tokenizer):
        ...
        emb = torch.from_numpy(
            self.get_glove_matrix(args.glove_path, tokenizer, initial_arr)
        )
...
class Tokenizer(object):
    ...
    def construct(self):
        ...
        if len(freq_sort) + len(self._idx2word) < self.vocab_size:
            print(
                "Actual vocab size smaller than that configured: {}/{}".format(
                    len(freq_sort) + len(self._idx2word), self.vocab_size
                )
            )
