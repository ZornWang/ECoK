...
from .dataset import FactLinkingDataset
...
from .utils.metrics import BLEU, METEOR, ROUGE
...
def train(
    ...
    if args.local_rank != -1:
        model = torch.nn.parallel.DistributedDataParallel(
            model,
            device_ids=[rank_to_device[args.local_rank]],
            output_device=rank_to_device[args.local_rank],
    ...
    for _ in train_iterator:
        ...
        if args.local_rank in [-1, 0]:
            ...
            with open(
                os.path.join(output_dir, args.params_file.split("/")[-1]), "w"
            ) as jsonfile:
                json.dump(args.params, jsonfile, indent=2)
            logger.info("Saving model checkpoint to %s", output_dir)

            validation_loss = results["avg_loss"]
            should_early_stop, best_path = early_stopper.early_stop(
                validation_loss, output_dir
            )
...
def evaluate(
    ...
    all_pred_ids = np.concatenate(
        [np.argmax(logits, axis=1).reshape(-1) for logits in all_preds]
    )
    all_pred_scores = np.concatenate(
        [softmax(logits, axis=1) for logits in all_preds], axis=0
    )
...
def main():
    ...
    if not args.eval_only:
        ...
        eval_dataset = dataset_class_eval(
            dataset_args, tokenizer, split_type=args.eval_dataset
        )
        ...
        if args.local_rank in [-1, 0]:
            ...
            with open(
                os.path.join(args.output_dir, args.params_file.split("/")[-1]), "w"
            ) as jsonfile:
