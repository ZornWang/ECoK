...
from .utils.data import pad_ids, truncate_sequences, truncate_sequences_dual
...
class FactLinkingDataset(torch.utils.data.Dataset):
    def __init__(self, args, tokenizer, split_type):
        ...
        with open(
            os.path.join(self.dataroot, f"neural_kg_data_{split_type}.json"), "r"
        ) as f:
            real_data = json.load(f)
        with open(
            os.path.join(self.dataroot, f"fake_data_{split_type}.json"), "r"
        ) as f:
            fake_data = json.load(f)

        self.encodings = self.tokenizer(
            real_data + fake_data, truncation=True, padding=True
        )
