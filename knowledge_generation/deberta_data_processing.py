...
for data_type in ["train", "val", "test"]:
    data_file = os.path.join(path, "json", f"{data_type}.json")
...
with open(os.path.join(path, "deberta", "unique_tails.json"), "w") as f:
...
with open(os.path.join(path, "deberta", "true_tails.json"), "w") as f:
...
for data_type in ["train", "val", "test"]:
    ...
    for i in tqdm(range(len(data[data_type]))):
        ...
        if len(unique_tails[data_type][relation]) <= (
            len(tails_in_use[head][relation]) + len(true_tails[head][relation])
        ):
        ...
        while (
            candidate_tail in true_tails[head][relation]
            or candidate_tail in tails_in_use[head][relation]
        ):
            candidate_tail = random.choice(unique_tails[data_type][relation])
...
print("Saving data")
for data_type in ["train", "val", "test"]:
    with open(os.path.join(path, "deberta", f"fake_data_{data_type}.json"), "w") as f:
        json.dump(fake_data[data_type], f)

# save unique head and relation pairs for COMET generation in jsonl format
for data_type in ["test"]:
    # path = "data_persona_gen"
    data_file = os.path.join(path, "json", "{}.json".format(data_type))
    ...
    with open(
        os.path.join(path, "deberta", "{}_unique.jsonl".format(data_type)),
        mode="w",
        encoding="utf-8",
    ) as f:
