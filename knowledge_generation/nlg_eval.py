...
def main():
    ...
    parser.add_argument(
        "--hypothesis_path", type=str, help="Path for json file of hypotheses."
    )
    parser.add_argument(
        "--references_path",
        type=str,
    ...
    avg_score_dict = {
        score_name: np.mean(np.array(score_list))
        for score_name, score_list in overall_scores_dict.items()
    }
    print(avg_score_dict)
    avg_score_dict = {
        score_name: float(avg_score) for score_name, avg_score in avg_score_dict.items()
    }
